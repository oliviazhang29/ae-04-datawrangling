...
It's more likely to find bookings in the City Hotels. 
...
hotels %>%
  ...
  arrange(desc(n))
```
It is surprising that there are 403 zero adults booking cases. Don't know how that happened.
...
```{r cancel-frequency}
hotels %>%
  filter(is_canceled == 1) %>% #canceled   
  count(adults)

hotels %>%
  filter(is_canceled == 0) %>% #canceled   
  count(adults)
...
hotels %>%
  filter(is_canceled == 0) %>% #not canceled
  count(adults) %>%
  arrange(desc(n))

hotels %>%
  count(adults, is_canceled == 1)
```
Among the 403 zero adults bookings, only 109 were canceled.
...
City hotel has higher adr on average. 
...
```{r get-unusual}
hotels %>%
  filter(adr == -6.38 | adr == 5400) %>% #why filter(adr == -6.38 | 5400) doesn't work
